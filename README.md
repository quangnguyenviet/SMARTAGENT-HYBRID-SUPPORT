...
### Quy trình hoạt động:
1.  **Giao tiếp**: Khách hàng tương tác qua Website (WebSocket) hoặc Facebook Messenger.
2.  **Tối ưu hóa**: Chat Widget trên Web sử dụng **Lazy Loading** (chỉ tạo hội thoại khi click) và **Session Persistence** để duy trì phiên chat.
3.  **Phân tích (Orchestrator)**: Mỗi tin nhắn được AI phân tích để chấm điểm tiềm năng (Lead Score) và nhận diện ý định (Intent).
4.  **Xử lý Lead**: Nếu điểm > 50, hệ thống yêu cầu thông tin liên hệ, gửi email thông báo duy nhất một lần cho chuyên viên.
5.  **Can thiệp (Take Over)**: Chuyên viên theo dõi Dashboard và có thể ngắt Bot để chat trực tiếp bất cứ lúc nào.
...
- `FB_PAGE_ACCESS_TOKEN`: Token của Facebook Page
- `FB_VERIFY_TOKEN`: Token xác thực Webhook
